pipeline {
    ...
    parameters {
      booleanParam defaultValue: false, description: 'Use npm to install dependent?', name: 'USE_DEPENDENT_MANAGER_INSTALL'
    }
    stages {
        stage('Build') {
            steps {
        ...
        stage('Deploy') {
            ...
            steps {
                ...
                sshPublisher(
                    publishers: [
                        sshPublisherDesc(
                            configName: env.ENV_PROD_SSH_CONFIG,
                            transfers: [
                                sshTransfer(
                                    cleanRemote: false,
                                    excludes: '',
                                    ...
                                    noDefaultExcludes: false,
                                    patternSeparator: '[, ]+',
                                    remoteDirectory: env.ENV_PROD_SERVER_TMP_ROOT,
                                    remoteDirectorySDF: false,
                                    removePrefix: '',
                                    sourceFiles: 'dist/app.7z'
                                )
                            ],
                            usePromotionTimestamp: false,
                            useWorkspaceInPromotion: false,
                    ...
                    publishers: [
                        ...
                        transfers: [
                            sshTransfer(
                                cleanRemote: false,
                                excludes: '',
                                execCommand: "cd $ENV_PROD_SERVER_TMP_ROOT && export BUILD_TAG=$BUILD_TAG && export SERVER_ROOT=$ENV_PROD_SERVER_ROOT && export SERVER_REAL_ROOT=$ENV_PROD_SERVER_REAL_ROOT && export SERVER_USER=$ENV_PROD_SERVER_USER && export SERVER_GROUP=$ENV_PROD_SERVER_GROUP && export SERVER_TMP_ROOT=$ENV_PROD_SERVER_TMP_ROOT && chmod +x deploy.sh && ./deploy.sh",
                                execTimeout: 120000,
                                flatten: false,
                                makeEmptyDirs: false,
                                noDefaultExcludes: false,
