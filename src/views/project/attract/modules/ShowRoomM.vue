<template>
  <a-modal
    :title="title"
    :width="1000"
    :visible="visible"
    :confirmLoading="confirmLoading"
    @cancel="handleCancel"
    cancelText="关闭"
    destroyOnClose
  >
    <a-card style="margin-bottom:10px">
      <a-form-item label="已选择房间">
        <a-input v-decorator="['title']" />
      </a-form-item>
    </a-card>
    <a-card>
      <a-form :form="form" layout="inline">
        <a-row>
          <a-col span="24">
            <a-form-item label="房间状态">
              <a-checkbox-group @change="searchQuery" v-model="queryParam.deptGroups">
                <a-checkbox value="1">已租</a-checkbox>
                <a-checkbox value="2">可租</a-checkbox>
              </a-checkbox-group>
              <!-- <a-input v-decorator="['title']" /> -->
            </a-form-item>
          </a-col>
          <a-col span="24">
            <a-form-item label="区块">
              <a-checkbox-group @change="loadDataBuilding" v-model="queryParam.deptGroups">
                <a-checkbox
                  v-for="item in buildingInfo.block"
                  :value="item.buildingProjectId"
                >{{item.projectName}}</a-checkbox>
              </a-checkbox-group>
            </a-form-item>
          </a-col>
          <a-col span="24">
            <a-form-item label="楼宇">
              <a-checkbox-group @change="searchQuery" v-model="queryParam.deptGroups">
                <a-checkbox value="1">已租{{}}</a-checkbox>
              </a-checkbox-group>
            </a-form-item>
          </a-col>
          <a-col span="24">
            <a-form-item label="楼层">
              <a-checkbox-group @change="searchQuery" v-model="queryParam.deptGroups">
                <a-checkbox value="1">已租{{}}</a-checkbox>
              </a-checkbox-group>
            </a-form-item>
          </a-col>
          <a-col span="24">
            <a-form-item label="房间">
              <a-checkbox-group @change="searchQuery" v-model="queryParam.deptGroups">
                <a-checkbox value="1">已租{{}}</a-checkbox>
              </a-checkbox-group>
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
    </a-card>
  </a-modal>
</template>

<script>
import { filterObj } from '@/utils/util'
import { getAction, putAction } from '@/api/manage'
import moment from 'moment'
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import Dom7 from 'dom7'
import { initDictOptions, filterDictText } from '@/components/dict/JDictSelectUtil'
import { mapGetters } from 'vuex'

export default {
  mixins: [JeecgListMixin],
  name: '',
  components: {},

  data() {
    return {
      form: this.$form.createForm(this),
      buildingInfo: {},
      confirmLoading: false,
      title: '物业位置',
      record: {},
      labelCol: {
        span: 18
      },
      wrapperCol: {
        span: 16
      },
      visible: false,
      loading: false,
      bodyStyle: {
        padding: '0',
        height: window.innerHeight * 0.8 + 'px',
        'overflow-y': 'auto'
      },
      queryform: {},
      modelStyle: {
        width: '60%',
        style: { top: '20px' },
        fullScreen: false
      },
      url: {
        list: '/park.project/mgrProjectTrace/list'
      }
    }
  },
  created() {},
  computed: {
    ...mapGetters(['industrialParkId'])
  },
  methods: {
    detail() {
      this.loadDataBlock()
    },
    // 1/5 点开modal自查
    loadDataBlock() {
      let params = { parkId: this.industrialParkId }
      this.loading = true
      getAction('/park.architecture/baseArchitectureProject/queryByParkId', params).then(res => {
        if (res.success) {
          this.buildingInfo.block = res.result
          console.log(this.buildingInfo.block)
          this.visible = true
        } else {
          this.$message.warning(res.message)
        }
        this.loading = false
      })
    },
    // 2/5 用block查building
    loadDataBuilding(record) {
      console.log(record)
      let params = { projectId: this.record.buildingProjectId }
      this.loading = true
      getAction('/park.architecture/baseArchitectureBuilding/queryByProjectId', params).then(res => {
        if (res.success) {
          this.buildingInfo.building = res.result
          console.log(this.buildingInfo.building)
        } else {
          this.$message.warning(res.message)
        }
        this.loading = false
      })
    },
    // 3/5 用building查floorName

    handleCancel() {
      this.visible = false
    }
  }
}
</script>

